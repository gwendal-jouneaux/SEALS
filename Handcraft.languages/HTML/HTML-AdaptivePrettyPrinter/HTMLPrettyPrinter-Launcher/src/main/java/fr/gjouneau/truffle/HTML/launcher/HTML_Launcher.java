package fr.gjouneau.truffle.HTML.launcher;
...
public final class HTML_Launcher {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            ...
            source = Source.newBuilder(HTML, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(HTML).in(in).out(out).options(options).build();
